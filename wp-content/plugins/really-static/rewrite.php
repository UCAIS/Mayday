<?php
add_filter ( 'rewrite_rules_array', 'really_static_insert_rewrite_rules' );
...
function home_urldddd($url, $path = "", $orig_scheme = "", $blog_id = "") {
	if (! really_static_selfdetect ())
		return $url;
		// S_LOG("url: $url");
		// S_LOG("A: ".loaddaten ( "realstaticlocalurl", 'reallystatic' ));
		
	// S_LOG("B: ".loaddaten ( "realstaticremoteurl", 'reallystatic' ));
		// S_LOG("C: ".get_option ( 'home' ) );
		
	// S_LOG("D: ".get_option ( 'siteurl' ));
		// S_LOG("E: ".str_replace( get_option ( 'home' )."/",loaddaten (
	// "realstaticremoteurl", 'reallystatic' ),$url));
	if ((get_option ( 'home' ) . "/") != loaddaten ( "realstaticremoteurl", 'reallystatic' ))
		return str_replace ( loaddaten ( "realstaticlocalurl", 'reallystatic' ), loaddaten ( "realstaticremoteurl", 'reallystatic' ), $url );
	else
		return $url;
}
...
function really_static_urlfix1($url) {
	if (! really_static_selfdetect ())
		return str_replace ( loaddaten ( "realstaticremoteurl", 'reallystatic' ), get_option ( 'home' ) . "/", $url );
	return $url;
