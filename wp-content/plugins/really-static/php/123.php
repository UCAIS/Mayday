...
if($_POST["pos"]==""){
	...
	';
		...
		echo "<br><h2>".__("STEP 2: set destinations", 'reallystatic')."</h2>";
		...
		if($_POST[test]!=""){
			...
			update_option ( 'realstaticlokalerspeicherpfad', REALLYSTATICHOME.'static/');
			update_option ( 'realstaticremoteurl', REALLYSTATICURLHOME.'static/');
			update_option("home",get_option('siteurl') );
			
			update_option ( 'realstaticdesignlocal', get_bloginfo('template_directory')."/");
		update_option ( 'realstaticdesignremote', get_bloginfo('template_directory')."/");
		update_option ( 'realstaticlocalurl',get_option('siteurl')."/");
		...
		#}
				...
				echo '<input name="realstaticremoteurl" type="text" size="90" value="'.loaddaten ( "realstaticremoteurl" , 'reallystatic').'">';
...
echo "<br>Have fun and please dont forget to donate!";
